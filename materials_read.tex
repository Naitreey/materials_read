...
\begin{enumerate}
    ...
    \item shell utils and commands:
        \begin{enumerate}
            ...
            \item vncviewer
            \item okular
            \item git (init|clone|branch|mv|status|remote|merge|mergetool|pull|fetch|push|commit|log|checkout|ls-files|rm|add|stash|config|diff|difftool|rebase|reset|tag|show), gitignore(5), svn (commit|checkout|log)
            \item mail
            \item sqlite3
            \item psql
