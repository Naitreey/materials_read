...
\begin{enumerate}
    ...
    \item Python Language Reference 2.7.10 --- Python official documentation (在读)
    \item Beginning Python From Novice to Professional (在读)
...
\begin{enumerate}
    ...
    \item shell utils and commands:
        \begin{enumerate}
            ...
            \item xinput, xclip,
            \item alternatives
            \item apt-get (install|update), dnf (install|remove|erease|update|updateinfo), yum, rpm, rpm
            ...
            \item screen
            \item df, du, fdisk, gdisk, parted, gparted, mkfs.<type>, fsck.<type>, dumpe2fs, tune2fs, debugfs, mount, umount, samba, findmnt, blkid, lsblk, smartctl, smartd.conf, /etc/fstab, sync, free, mkswap, swapon, swapoff, dmsetup, lspci, ionice
            ...
            \item hostname
            \item gcc, gdb, ctags, cscope, gvim, vim, gvimdiff, python, python3, pip, pip3, pydoc, java, javac, make, diff, patch, ldd, strings, pmap, taskset
            \item ping, arping, iptables
            \item virsh (list|start|destroy), qemu-img
            \item vncviewer
            \item okular
            \item git (init|clone|branch|mv|status|remote|merge|mergetool|pull|fetch|push|commit|log|checkout|ls-files|rm|add|stash|config|diff|difftool|rebase|reset|tag|show|instaweb|clean|cherry-pick|reflog), git-shell, gitignore(5), gitrevisions(7)
...
\begin{enumerate}
    ...
    \item HTML and XHTML: The Definitive Guide (已读无笔记, 可能不会再读了)
\end{enumerate}
