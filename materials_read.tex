...
\begin{enumerate}
    ...
    \item shell utils and commands:
        \begin{enumerate}
            ...
            \item vncviewer
            \item okular
            \item git (init|clone|branch|mv|status|remote|ls-remote|merge|mergetool|merge-base|pull|fetch|push|commit|log|shortlog|checkout|ls-files|rm|add|stash|config|diff|difftool|apply|rebase|reset|tag|show|instaweb|clean|cherry-pick|cherry|reflog|request-pull|format-patch|am|send-email|rev-parse|rerere|describe|archive|gc|help), git-shell, gitignore(5), gitrevisions(7)
